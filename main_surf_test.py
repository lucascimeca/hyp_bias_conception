...
def run_experiment(args, experiments=None, dsc=None, samples=10, scope=None):
    ...
    for exp_key in experiments.keys():
        ...
        if args.arch.endswith('resnet'):
            ...
            state_dict = torch.load("./models/pretrained/resnet-{}.pth".format(exp_key))
            ...
            model = ConvNet(
                num_classes=num_classes,
                no_of_channels=no_of_channels)
            state_dict = torch.load("./models/pretrained/convnet-{}.pth".format(exp_key))
            state_dict_rex = {key[7:]:state_dict[key] for key in state_dict.keys()}
            model.load_state_dict(state_dict_rex)
            ...
            model = FFNet(
                num_classes=num_classes,
                no_of_channels=no_of_channels)
            state_dict = torch.load("./models/pretrained/ffnet-{}.pth".format(exp_key))
            state_dict_rex = {key[7:]:state_dict[key] for key in state_dict.keys()}
            model.load_state_dict(state_dict_rex)
            ...
            state_dict = torch.load("./models/pretrained/vit-{}.pth".format(exp_key))
            state_dict_rex = {key[7:]:state_dict[key] for key in state_dict.keys()}
            model.load_state_dict(state_dict_rex)
        else:
        ...
        dir_file = DIRECTION_FILES_FOLDER + "/direction_{}".format(exp_key) # name the direction file
...
if __name__ == '__main__':
    ...
    # experiments
    experiments = {
        "color_augmentation": ('color', 'shape', 'scale', 'orientation'),
        "shape_augmentation": ('color', 'shape', 'scale', 'orientation'),
        "orientation_augmentation": ('color', 'shape', 'scale', 'orientation'),
        "scale_augmentation": ('color', 'shape', 'scale', 'orientation'),
        "diagonal": ('color', 'shape', 'scale', 'orientation'),
    }
