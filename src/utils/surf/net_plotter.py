...
from utils.surf import h5_util
...
def get_weights(net):
    ...
    return [p.data.cpu() for p in net.parameters()]
...
def name_surface_file2(args, dir_file):
...
def setup_direction2(dir_file, net):
    """
    ...
    f.close()
    print("direction file created: %s" % filename)


def setup_surface_file(args, surf_file, dir_file):
    # skip if the direction file already exists
    if file_exists(surf_file):
        remove_file(surf_file)
